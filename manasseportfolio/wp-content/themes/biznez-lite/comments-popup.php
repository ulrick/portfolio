...
<p><?php _e('No comments yet.','biznez'); ?></p>
<?php } ?>
<?php if ('open' == $post->comment_status) { ?>
<h2><?php _e('Leave a comment','biznez'); ?></h2>
<p><?php _e('Line and paragraph breaks automatic, e-mail address never displayed, <acronym title="Hypertext Markup Language">HTML</acronym> allowed: ','biznez');?><code><?php echo allowed_tags(); ?></code></p>
<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
  <?php if ( $user_ID ) : ?>
  <p><?php _e('Logged in as ','biznez'); ?><a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account">Log out &raquo;</a></p>
  <?php else : ?>
  <p> <span class="left-label">
    <label for="author"><?php _e('Name','biznez'); ?></label>
  ...
  <p> <span class="left-label">
    <label for="email"><?php _e('E-mail','biznez'); ?></label>
    </span> <span class="right-input">
  ...
  <p> <span class="left-label">
    <label for="url"><abbr title="Universal Resource Locator"><?php _e('URL','biznez'); ?></abbr></label>
    </span> <span class="right-input">
    <input type="text" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>" size="28" tabindex="3" />
    </span> <span class="clear"></span> </p>
  <?php endif; ?>
  <p> <span class="left-label">
    <label for="comment"><?php _e('Your Comment','biznez'); ?></label>
    </span> <span class="right-input">
    <textarea name="comment" id="comment" cols="33" rows="4" tabindex="4"></textarea>
    </span> <span class="clear"></span> </p>
  <p> <span class="input">
    <input name="submit" type="submit" tabindex="5" value="<?php esc_attr_e('Say It!','biznez') ?>" />
    </span> <span class="clear"></span> </p>
  <?php do_action('comment_form', $post->ID); ?>
</form>
<?php } else { // comments are closed ?>
<p><?php _e('Sorry, the comment form is closed at this time.','biznez'); ?></p>
<?php }
} // end password check?>
<div><strong><a href="javascript:window.close()"><?php _e('Close this window.','biznez'); ?></a></strong></div>
