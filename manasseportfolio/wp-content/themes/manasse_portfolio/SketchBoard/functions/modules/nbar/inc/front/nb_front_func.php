...
<style type="text/css">
#notificationbar{<?php if(isset($nbar['defaultposition'])){ echo $nbar['defaultposition']; } ?>:0;}
#notificationbar,#nbar_downArr{background-color:<?php echo $nbar['colorScheme']; ?>;} 
#nbar_downArr{<?php if(isset($nbar['defaultposition']) =="top"){ ?>margin-top:4px;<?php } else { echo "top:-26px";  } ?>}
...
<div id="notificationbar" class="<?php echo $nbar_option; ?>">
	<div class="nbar_wrapper <?php if(isset($nbarH['defaultposition'])){ echo $nbarH['defaultposition']; }?>">
		...
		<div class="nbar_extendmsg <?php echo $nbarH['extendMesgTemplate']; ?>">
			<div class="nbar_exmsg">
				<?php if($nbarH['extendMesgImg']){ ?><img src="<?php echo $nbarH['extendMesgImg']; ?>" class="nbar_extendmsg_img" alt="nbar-img" /><?php } ?>
	...
	<a id="nbar_downArr" class="<?php if(isset($nbarH['defaultposition'])){ echo $nbarH['defaultposition']; } ?>" href="JavaScript:void(0);" >+</a>
</div>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		...
		<?php 
		if(isset($nbar_opts['defaultposition']) =="bottom"){ ?>jQuery('body').append('<div class="notification_push"></div>');<?php } 
		else{ ?>jQuery('body').prepend('<div class="notification_push"></div>');<?php } ?>
		
		jQuery("#notificationbar").notificationbar({
		'defaultstate': '<?php if(isset($nbar_opts['defaultState'])){ echo $nbar_opts['defaultState']; } ?>',
